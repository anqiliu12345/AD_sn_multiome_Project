...
# Load data
sce1 <- readH5AD(paste0("/work/aliu10/AD_sc_Project/results/", case_group, "/adata_", case_group, ".h5ad"))
sce2 <- readH5AD(paste0("/work/aliu10/AD_sc_Project/results/", control_group, "/adata_", control_group, ".h5ad"))

sce1$Status <- case_group
sce2$Status <- control_group

burkhardt_sce <- cbind(sce1, sce2)
...
burkhardt.identity <- data.frame(group = burkhardt.meta.data$CellType, row.names = row.names(burkhardt.meta.data))

control.use <- rownames(burkhardt.meta.data)[burkhardt.meta.data$Status == case_group] # extract the cell names from disease data
stim.use <- rownames(burkhardt.meta.data)[burkhardt.meta.data$Status == control_group] 

control.data.input <- burkhardt.data.input[, control.use]
stim.data.input <- burkhardt.data.input[, stim.use]

control.meta <- burkhardt.meta.data[burkhardt.meta.data$Status == case_group, ]
stim.meta <- burkhardt.meta.data[burkhardt.meta.data$Status == control_group, ]

burkhardt.control.identity <- data.frame(group = control.meta$CellType, row.names = row.names(control.meta))
burkhardt.stim.identity <- data.frame(group = stim.meta$CellType, row.names = row.names(stim.meta))
...
burkhardt.control.cc <- smoothData(burkhardt.control.cc, adj = PPI.human) # Other option includes PPI.human We're told that we may have to comment these out.
...
burkhardt.stim.cc <- smoothData(burkhardt.stim.cc, adj = PPI.human) # Other option includes PPI.human We're told that we may have to comment these out.
...
write.csv(burkhardt.control.net, file = paste0("/work/aliu10/AD_sc_Project/results/CCI/seaAD_communications_case_", compare_groups, ".csv"))
write.csv(burkhardt.stim.net, file = paste0("/work/aliu10/AD_sc_Project/results/CCI/seaAD_communications_control_", compare_groups, ".csv"))

# Save RDS files
saveRDS(burkhardt.stim.cc, file = paste0("/work/aliu10/AD_sc_Project/results/CCI/seaAD_communications_case_", compare_groups, ".rds"))
saveRDS(burkhardt.control.cc, file = paste0("/work/aliu10/AD_sc_Project/results/CCI/seaAD_communications_control_", compare_groups, ".rds"))
